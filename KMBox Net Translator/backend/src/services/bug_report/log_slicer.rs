...
//! the same UTC day boundary (`kmbox.YYYY-MM-DD.log`). Both ends are
...
/// Build `<logs_dir>/kmbox.YYYY-MM-DD.log` if the file exists; `None`
/// otherwise.
fn log_path_for(logs_dir: &Path, date: NaiveDate) -> Option<PathBuf> {
    let name = format!("kmbox.{}.log", date.format("%Y-%m-%d"));
...
mod tests {
    ...
    fn write_log(dir: &Path, date: NaiveDate, body: &str) {
        let p = dir.join(format!("kmbox.{}.log", date.format("%Y-%m-%d")));
