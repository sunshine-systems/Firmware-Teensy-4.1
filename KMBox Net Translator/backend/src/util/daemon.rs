...
pub const PID_FILE_NAME: &str = "kmbox_net_translator.pid";

/// Filename written to [`std::env::temp_dir`] holding our bound UDP port.
pub const PORT_FILE_NAME: &str = "kmbox_net_translator.port";
...
pub const HTTP_PORT_FILE_NAME: &str = "kmbox_net_translator.http_port";
...
/// On Windows the image name is `kmbox_net_translator.exe`; on other
/// platforms it's `kmbox_net_translator`. The substring catches both.
const PROCESS_NAME_NEEDLE: &str = "kmbox_net_translator";
...
pub fn takeover_if_running() -> TakeoverOutcome {
    ...
    // `kmbox_net_translator.exe`); doing a substring match catches
