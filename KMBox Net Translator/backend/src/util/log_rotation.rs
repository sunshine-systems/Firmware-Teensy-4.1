...
//! `<logs_dir>/kmbox*.log` every hour, sums the file sizes, and trims
...
/// 1 GiB total cap across all `kmbox*.log` files in the logs dir.
...
///
/// Enumerates every `kmbox*.log` in `logs_dir`, sums sizes, and if the
...
/// True for files matching `kmbox*.log`. We match on filename only so
/// rogue files (e.g. `kmbox.log.swp`) don't get swept.
fn is_kmbox_log(path: &Path) -> bool {
    let Some(name) = path.file_name().and_then(|s| s.to_str()) else {
        return false;
    };
    name.starts_with("kmbox") && name.ends_with(".log")
}
...
mod tests {
    ...
    #[test]
    fn is_kmbox_log_filters_correctly() {
        assert!(is_kmbox_log(Path::new("kmbox.2025-01-01.log")));
        assert!(is_kmbox_log(Path::new("/tmp/kmbox.log")));
        assert!(is_kmbox_log(Path::new("kmbox-2025-01-01.log")));
        assert!(!is_kmbox_log(Path::new("other.log")));
        assert!(!is_kmbox_log(Path::new("kmbox.log.bak")));
        assert!(!is_kmbox_log(Path::new("kmbox.2025-01-01.log.swp")));
    }
}
