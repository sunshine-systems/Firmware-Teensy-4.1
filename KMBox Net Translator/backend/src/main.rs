...
/// at `<data_dir>/logs/kmbox.YYYY-MM-DD.log` alongside the stdout layer.
///
...
/// outlive the program.
///
/// Filename pattern: `kmbox.YYYY-MM-DD.log` — `tracing-appender`'s
/// `RollingFileAppender::builder()` joins prefix + date + suffix with
/// `'.'` separators, producing exactly that form when we pass `"kmbox"`
/// and `"log"`.
...
fn run(
    ...
    // kmbox*.log files when the total exceeds the hardcoded 1 GiB cap.
