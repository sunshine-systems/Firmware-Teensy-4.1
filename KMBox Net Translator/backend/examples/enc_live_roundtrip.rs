...
//! NET):` log lines (in `%LOCALAPPDATA%/KMBox Net Translator/logs/`)
//! carry the decoded button / move / wheel values — operator should
//! `grep "IN (KMBOX NET):" $LOCALAPPDATA/KMBox Net Translator/logs/kmbox.*.log`
...
use kmbox_net_translator::kmbox_net::encryption::{encrypt, key_from_mac, ENC_PACKET_LEN};
use kmbox_net_translator::kmbox_net::{
