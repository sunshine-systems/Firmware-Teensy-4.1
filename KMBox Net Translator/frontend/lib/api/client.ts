...
// `window.kmbox`; falls back to a plain `fetch` against the local
...
/** Shape returned by `window.kmbox.getBackendUrl()`. Never throws. */
export type BridgeBackendUrlResult =
  | { ok: true; http: string; ws: string; port: number }
  | { ok: false; reason: "no_port_file" };

/**
 * Shape returned by `window.kmbox.healthCheck()`. Never throws. The
...
/** Shape returned by `window.kmbox.healthDetail()`. Never throws. */
 ...
 * this file.
 */
export function getBridge(): KmboxBridge | null {
  if (typeof window === "undefined") return null;
  return window.kmbox ?? null;
}
 ...
 *
 * The dev port comes from `NEXT_PUBLIC_KMBOX_HTTP_PORT` — Next.js
...
export async function fallbackBugReport(): Promise<BridgeBugReportResult> {
  const port = process.env.NEXT_PUBLIC_KMBOX_HTTP_PORT;
  if (!port) {
    throw new BugReportError(
      "http_port_unavailable",
      "no NEXT_PUBLIC_KMBOX_HTTP_PORT set and not running inside Electron"
    );
  }
  let resp: Response;
  try {
  ...
  a.download = m ? m[1] : "kmbox_bug_report.zip";
