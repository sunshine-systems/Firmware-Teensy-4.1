...
published to `%TEMP%\kmbox_net_translator.http_port` so the Electron
...
| Entry | Source | Notes |
|---|---|---|
| `kmbox_last_5min.log` | `log_slicer::slice_last_window` | Tail of today's + yesterday's daily log file, timestamp-filtered to the 5-minute window. ANSI escapes stripped defensively. |
...
- **Takeover** — on startup the daemon checks
  `%TEMP%\kmbox_net_translator.pid`. If it names a live process whose
  image name matches `kmbox_net_translator`, the new instance
...
- **Publication** — after successful UDP bind, the daemon atomically
  ...
  `%TEMP%\kmbox_net_translator.port`. The HTTP port is published
  separately to `%TEMP%\kmbox_net_translator.http_port` after the
  axum bind succeeds.
...
`<data_dir>/logs/kmbox.YYYY-MM-DD.log`; a separate hourly janitor
