...
const PID_FILE = path.join(os.tmpdir(), 'kmbox_net_translator.pid');
const HTTP_PORT_FILE = path.join(os.tmpdir(), 'kmbox_net_translator.http_port');
const PORT_FILE = path.join(os.tmpdir(), 'kmbox_net_translator.port');
const DAEMON_IMAGE_NEEDLE = 'kmbox_net_translator';
...
function readExistingDaemonPid() {
  try {
    ...
    if (!isOurDaemon(pid)) {
      removeStaleFiles(`pid ${pid} alive but not kmbox_net_translator`);
      return null;
    }
    return pid;
  } catch (err) {
...
function spawnIfNeeded(opts) {
  ...
  // use this today only for KMBOX_FRONTEND_DIR (so the Rust daemon
