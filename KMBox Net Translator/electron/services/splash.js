...
// against a tiny inline `window.kmboxSplash` API in splash.html.
//
...
function setStatus(text) {
  ...
  const js = 'window.kmboxSplash && window.kmboxSplash.setStatus(' + JSON.stringify(String(text)) + ')';
...
function beginClose() {
  if (!splashWin || splashWin.isDestroyed() || destroyed) return;
  const js = 'window.kmboxSplash && window.kmboxSplash.beginClose()';
