...
* `cargo build --release` — clean, `target/release/kmbox_net_translator.exe` rebuilt.
...
## Daemon shutdown
After the walk completed I `taskkill /F /PID 6956`'d the background daemon. The stale `kmbox_net_translator.{pid,port,http_port}` files in `%TEMP%` were removed manually so the next start of the daemon doesn't have to wait for the takeover-of-dead-pid path.
